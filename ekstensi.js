...
function auto_safeconvert() { 
	...
	if (protected\_links != "" && !protected\_links.match(a\_to\_vd)) { protected\_links += ", " + a\_to_vd; } 
	else if (protected_links == "") { protected\_links = a\_to_vd; } 
	var a\_to\_ve = ""; 
	...
	for (var i = 0; i < a\_to\_va; i++) { 
		...
		while (a\_to\_vh == false && j < a\_to\_vg) { 
			...
			if (a\_to\_vi.match(a\_to\_vf\[j\]) || !a\_to\_vi || !a\_to\_vi.match("http")) { a\_to\_vh = true; } 
			j++; 
		} 
		if (a\_to\_vh == false) { 
			var encryptedUrl = Base64.encode(a\_to\_vi); 
			a\_to\_ve\[i\].href = "https://omgmotor.com/2578-2/" + encryptedUrl; 
	...
	if (a\_to\_vj) { a\_to\_vj.innerHTML += a\_to\_vb; } 
	if (a\_to\_vk) { a\_to\_vk.innerHTML += a\_to\_va; } 
} 
...
<%- __("from") %>: <%- config.title %>
<%- __("author") %>: <%- config.author %>
<%- __("link") %>: https://www.digimoclub.com/tutorial-safelink-wordpress/
